...
public class EmailServiceInterop {
	...
	public Response recuperarEmails() {
		...
		URI uri = UriBuilder.fromPath("/cervejaria/services")
				.path(EmailServiceInterop.class).path("/{id}").build(emailId);

		Link link = Link.fromUri(uri).build();

		return Response.accepted().header("Location", link.getUri())
				.header("Expires", calendar.getTime()).build();
	}
