...
public class TwitterLoginFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		if (accessToken == null) {
			...
			String token = req
					.getParameter(TwitterOAuthFlowService.OAUTH_TOKEN_FIELD);

			if (verifier != null && token != null) {
				accessToken = TwitterOAuthFlowService.verify(token, verifier,
						req);
	...
	private void ajustaCookiesNaResposta(HttpServletResponse resp,
		...
		Cookie accessTokenSecretCookie = new Cookie(TOKEN_COOKIE_SECRET,
